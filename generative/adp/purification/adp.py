import torch
import sys
from generative.adp.utils import *
...
def adp(x, network_ebm, max_iter, mode, config):
    ...
    images.append(x_pur)
    x_pur = transform_raw_to_ebm(x_pur).cuda()

    cont_purification = torch.ones(x_pur.shape[0], dtype=torch.bool).cuda()
    # Stopping criterion
    for i in range(max_iter):
        labels = torch.ones(x_pur.shape[0]).cuda()
        ...
        x_pur = torch.clamp(transform_ebm_to_raw(x_pur + grad * step_size[:, None, None, None]), 0.0, 1.0)
